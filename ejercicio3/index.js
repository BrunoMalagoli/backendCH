...
class Contenedor {
  ...
  async getAll() {
    try {
      const allContent = await fs.promises.readFile(
        `./${this.fileName}`,
        "utf-8",
        (error) => {
          if (error) {
  ...
  async deleteById(id) {
    try {
      ...
      let dataExcluded = await fs.promises.writeFile(
        `./${this.fileName}`,
...
const prod1 = {
  ...
  price: 7.21,
  thumbnail:
    "https://images.unsplash.com/photo-1529651795107-e5a141e34843?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=870&q=80",
};
const prod2 = {
  title: "Regla",
  price: 220.21,
